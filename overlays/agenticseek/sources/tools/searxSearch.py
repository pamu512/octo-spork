...
from sources.tools.searx_query_policy import sanitize_searx_query, want_strict_sanitization
...
class searxSearch(Tools):
    def __init__(self, base_url: str = None):
        ...
        if not self.base_url:
            raise ValueError(
                "SearxNG base URL must be provided either as an argument or via "
    ...
    def execute(self, blocks: list, safety: bool = False) -> str:
        """Executes a search query against a SearxNG instance using POST."""
        if not blocks:
        ...
        if want_strict_sanitization():
            query = sanitize_searx_query(query)
            if not query:
                return (
                    "Error: Search query was empty after privacy redaction "
                    "(avoid pasting tokens or repo-identifying strings into web search)."
                )
        ...
        safe = "2" if want_strict_sanitization() else "0"
        data = urlencode(
            {
                "q": query,
                "categories": "general",
        ...
        try:
            ...
            span_cm = mgr.tool_span(
                "searxSearch",
                attributes={
                    "searx.query": query[:800],
                    "searx.base_url": (self.base_url or "")[:256],
                },
            )
        except ImportError:
            pass
